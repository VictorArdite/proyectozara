...
function App() {
  ...
  return (
    <div className="bg-white min-h-screen flex flex-col font-serif">
      ...
      {/* Header */}
      <header className="fixed top-0 left-0 w-full z-30 bg-white/90 backdrop-blur border-b border-neutral-200 flex justify-between items-center px-8 py-4">
        {/* Logo */}
      ...
      <main className="flex-1 flex flex-col items-center justify-center pt-28">
        ...
        <section className="w-full flex flex-col items-center mb-14">
          <img
            ...
            alt="Zara Main"
            className="rounded-2xl shadow-lg w-full max-w-4xl aspect-[16/7] object-cover mb-10"
          />
          <h1 className="text-5xl md:text-7xl font-serif font-bold text-center mb-5 tracking-tighter">
          ...
          <button
            className="bg-black text-white uppercase px-8 py-3 rounded-2xl font-semibold tracking-wider shadow hover:bg-neutral-800 transition"
          >
            Comprar ahora
          </button>
        </section>
          ...
          {items.map((item, i) => (
            ...
            >
              <img
                src={item.image}
                alt={item.title}
                className="rounded-2xl shadow-md w-full md:w-1/2 aspect-[4/5] object-cover hover:scale-105 hover:shadow-xl transition"
              />
              <div className="flex flex-col gap-4 items-start md:items-start md:w-1/2 px-2">
                ...
                <motion.button
                  ...
                  className="mt-4 bg-black text-white px-7 py-2 rounded-full uppercase tracking-wide font-medium hover:bg-neutral-800 transition"
                >
                  {item.button}
                </motion.button>
              </div>
